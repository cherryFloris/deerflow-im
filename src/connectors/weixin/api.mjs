...
// Local reimplementation of dsh-im's shared `fetchImageBuffer` (no external dep).
// Downloads an image/file URL into a Buffer for AES-128-ECB decryption.
async function fetchImageBuffer(url, { signal, headers } = {}) {
  const r = await fetch(url, { signal, headers });
  if (!r.ok) throw new Error(`weixin media download failed: ${r.status}`);
